from typing import List, Optional
from fastapi import APIRouter, Depends, status, Query
from sqlalchemy.orm import Session
import uuid

from app.core.database import get_db_session
from app.core.deps import get_current_user
from app.models.user import User
from app.services.role import role_service
from app.schemas.role import Role, RoleCreate, RoleUpdate
from app.core.logger import get_logger

logger = get_logger("roles.api")
router = APIRouter()

@router.post("/", response_model=Role, status_code=status.HTTP_201_CREATED)
async def create_role(
    *,
    db: Session = Depends(get_db_session),
    role_in: RoleCreate,
    current_user: User = Depends(get_current_user)
):
    """
    创建角色
    """
    return await role_service.create_role(db, role_in)

@router.get("/", response_model=List[Role])
async def read_roles(
    db: Session = Depends(get_db_session),
    skip: int = Query(0, ge=0, description="跳过的记录数"),
    limit: int = Query(100, ge=1, le=1000, description="返回的记录数"),
    status: Optional[str] = Query(None, description="角色状态筛选"),
    current_user: User = Depends(get_current_user)
):
    """
    获取角色列表
    """
    return await role_service.get_roles(db, skip=skip, limit=limit, status_filter=status)

@router.get("/{role_id}", response_model=Role)
async def read_role(
    role_id: uuid.UUID,
    db: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user)
):
    """
    根据ID获取角色
    """
    return await role_service.get_role_by_id(db, role_id)

@router.put("/{role_id}", response_model=Role)
async def update_role(
    role_id: uuid.UUID,
    role_update: RoleUpdate,
    db: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user)
):
    """
    更新角色
    """
    return await role_service.update_role(db, role_id, role_update)

@router.delete("/{role_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_role(
    role_id: uuid.UUID,
    db: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user)
):
    """
    删除角色
    """
    await role_service.delete_role(db, role_id)
    return None

@router.patch("/{role_id}/status", response_model=Role)
async def change_role_status(
    role_id: uuid.UUID,
    status: str = Query(..., regex="^[01]$", description="状态：0-禁用，1-启用"),
    db: Session = Depends(get_db_session),
    current_user: User = Depends(get_current_user)
):
    """
    更改角色状态
    """
    return await role_service.change_role_status(db, role_id, status)

@router.get("/count/total")
async def get_role_count(
    db: Session = Depends(get_db_session),
    status: Optional[str] = Query(None, description="角色状态筛选"),
    current_user: User = Depends(get_current_user)
):
    """
    获取角色总数
    """
    count = await role_service.get_role_count(db, status_filter=status)
    return {"total": count} 